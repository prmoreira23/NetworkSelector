...
def calculate_QoS(iface="wlan0", end='8.8.8.8'):
    #check_args()
    check_privileges()

    ip     = end
    pings  = 100#int(sys.argv[2])
    interval = 0.001

    ping_c = "ping -c {0} -i {1} -I {2} {3} > pings_{4}.raw".format(pings, interval, iface, ip, iface)
    trans_c = "cat pings_%s.raw | grep packets\ transmitted | awk '{print $1}'" % (iface)
    recv_c = "cat pings_%s.raw | grep packets\ transmitted | awk '{print $4}'" % (iface)
    stats_c = "cat pings_%s.raw | grep rtt | awk '{print $4}'" % (iface)
    time_c = "cat pings_%s.raw | grep packets\ transmitted | awk '{print $10}'" % (iface)
